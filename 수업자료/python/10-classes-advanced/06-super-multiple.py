...
> MRO에 따라, 현재 클래스의 부모(상위) 클래스의 메서드나 속성에 접근할 수 있게 해주는 특수한 객체를 반환하는 내장 함수

- 단일 상속에서는
  - “부모 클래스의 생성자(또는 메서드)를 호출하기 위해 super()를 사용한다.”
- 다중상속에서는
  - "MRO에 따라 각 클래스의 메서드를 찾아가기 때문에, 단순히 직계 부모만이 아니라 다중 상속 관계에서도 적절한 상위 클래스의 메서드를 명확하게 호출할 수 있음"
"""
